...
message count from the remote server.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if cmd.Flag("verbose").Value.String() == "true" {
			...
			log.Stats("Memory in use", humanize.Bytes(uint64(stats.MemoryAllocated)))
			log.Stats("Lookups", fmt.Sprintf("%d", stats.Lookups))
			log.Stats("Next GC at", humanize.Bytes(uint64(stats.NextGC)))
			log.Stats("File store count", fmt.Sprintf("%d", stats.FileStoreCount))
			log.Stats("File store size", humanize.Bytes(uint64(stats.FileStoreSize)))
		}
	},
}
